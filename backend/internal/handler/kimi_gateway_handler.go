...
// KimiGatewayHandler handles Kimi For Coding chat completions requests
type KimiGatewayHandler struct {
	kimiGatewayService      *service.KimiGatewayService
...
// NewKimiGatewayHandler creates a new KimiGatewayHandler
func NewKimiGatewayHandler(
	...
	return &KimiGatewayHandler{
		kimiGatewayService:      kimiGatewayService,
...
// ChatCompletions handles Kimi /v1/chat/completions endpoint
func (h *KimiGatewayHandler) ChatCompletions(c *gin.Context) {
	...
	logger.FromContext(ctx).Info("kimi.chat_completions",
	...
	if err := h.billingCacheService.CheckBillingEligibility(ctx, apiKey.User, apiKey, apiKey.Group, subscription); err != nil {
		logger.FromContext(ctx).Warn("kimi.billing_eligibility_check_failed", zap.Error(err))
	...
	defer func() {
		if closeErr := c.Request.Body.Close(); closeErr != nil {
			logger.FromContext(ctx).Warn("kimi.close_request_body_failed", zap.Error(closeErr))
		}
	}()
	...
	logger.FromContext(ctx).Info("kimi.request_parsed",
	...
	if h.gatewayService != nil {
		parsedRequest, err := service.ParseGatewayRequest(body, service.PlatformKimi)
		if err == nil {
	...
	for i := 0; i <= maxSwitches; i++ {
		// Select Kimi account
		account, err := h.kimiGatewayService.SelectAccount(ctx, apiKey.GroupID, sessionHash)
		if err != nil {
			logger.FromContext(ctx).Warn("kimi.account_select_failed", zap.Error(err), zap.Int("attempt", i))
		...
		// Forward request
		var result *service.KimiForwardResult
		if stream {
		...
		if h.isRetryableError(err) {
			...
			if sessionHash != "" && i == 0 {
				...
				_ = h.kimiGatewayService.DeleteStickySessionAccountID(ctx, groupID, sessionHash)
			}
			logger.FromContext(ctx).Warn("kimi.forward_failed_retryable",
				zap.Error(err),
		...
		if h.errorPassthroughService != nil && result != nil {
			...
			logger.FromContext(ctx).Error("kimi.forward_failed",
				zap.Error(err),
				zap.Int64("account_id", account.ID),
			)
...
func (h *KimiGatewayHandler) forwardNonStream(c *gin.Context, account *service.Account, body []byte) (*service.KimiForwardResult, error) {
	result, err := h.kimiGatewayService.Forward(c.Request.Context(), account, body)
	if err != nil {
		return nil, err
	}
	...
	// Log response
	h.kimiGatewayService.LogResponse(c.Request.Context(), account.ID, result.StatusCode, int(duration))
...
func (h *KimiGatewayHandler) forwardStream(c *gin.Context, account *service.Account, body []byte) (*service.KimiForwardResult, error) {
	result, err := h.kimiGatewayService.ForwardStream(c.Request.Context(), account, body, c.Writer)
	if err != nil {
		return nil, err
	}
	...
	logger.FromContext(c.Request.Context()).Info("kimi.stream_completed",
		zap.Int64("account_id", account.ID),
...
// Models returns the list of available Kimi models
func (h *KimiGatewayHandler) Models(c *gin.Context) {
	models := h.kimiGatewayService.ListModels()
...
func (h *KimiGatewayHandler) isRetryableError(err error) bool {
	if err == nil {
		return false
	}
...
func (h *KimiGatewayHandler) handleBillingError(c *gin.Context, err error) {
...
func (h *KimiGatewayHandler) errorResponse(c *gin.Context, status int, code, message string) {
	response.Error(c, status, message)
}

// applyErrorPassthrough 应用错误透传规则到错误响应
func (h *KimiGatewayHandler) applyErrorPassthrough(
	c *gin.Context,
	upstreamStatus int,
	responseBody []byte,
	defaultStatus int,
	...
	rule := h.errorPassthroughService.MatchRule(service.PlatformKimi, upstreamStatus, responseBody)
	if rule == nil {
		return defaultStatus, defaultCode, defaultMessage
	}
...
func (h *KimiGatewayHandler) submitUsageRecordTask(task service.UsageRecordTask) {
	...
	defer func() {
		if recovered := recover(); recovered != nil {
			logger.L().With(
				zap.String("component", "handler.kimi_gateway"),
			).Error("kimi.usage_record_task_panic_recovered")
		}
	}()
	task(ctx)
}
...
func (h *KimiGatewayHandler) recordUsage(c *gin.Context, apiKey *service.APIKey, account *service.Account, model string, result *service.KimiForwardResult, subscription *service.UserSubscription) {
	if result.Usage == nil {
		logger.FromContext(c.Request.Context()).Warn("kimi.record_usage_nil",
			zap.Int64("api_key_id", apiKey.ID),
			zap.Int64("account_id", account.ID),
			zap.String("model", model),
		)
	...
	logger.FromContext(c.Request.Context()).Info("kimi.record_usage_start",
		zap.Int64("api_key_id", apiKey.ID),
		zap.Int64("account_id", account.ID),
		zap.String("model", model),
	...
	h.submitUsageRecordTask(func(ctx context.Context) {
		...
		if err := h.kimiGatewayService.RecordUsage(ctx, &service.KimiRecordUsageInput{
			APIKey:        apiKey,
			...
			logger.FromContext(ctx).Error("kimi.record_usage_failed",
				zap.Error(err),
				zap.Int64("api_key_id", apiKey.ID),
				zap.Int64("account_id", account.ID),
			)
