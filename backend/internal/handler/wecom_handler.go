...
func (h *WeComHandler) buildReplyMessage(invitationCode, redeemCode string) string {
	...
	_, _ = b.WriteString("教程地址为：")
	_, _ = b.WriteString(guideURL)
	_, _ = b.WriteString("（请复制到浏览器打开，内含详细使用方法）\n")
	_, _ = b.WriteString("注册邀请码：")
	_, _ = b.WriteString(invitationCode)
	_, _ = b.WriteString("\n兑换码：")
	_, _ = b.WriteString(redeemCode)
	_, _ = b.WriteString("\n查看用量地址：")
	_, _ = b.WriteString(usageURL)
	_, _ = b.WriteString(" （请复制到浏览器打开，内含详细使用方法）")
	_, _ = b.WriteString("\n")
	_, _ = b.WriteString(notice)
	_, _ = b.WriteString("\n注意： ")
	_, _ = b.WriteString(disclaimer)
	return b.String()
}
