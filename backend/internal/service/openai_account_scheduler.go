...
func (s *defaultOpenAIAccountScheduler) selectByLoadBalance(
	...
	if len(candidates) > 0 {
		...
		for i := range candidates {
			...
			// When requireCompact, give tier=2 accounts a large score boost
			if req.RequireCompact {
				tier := openAICompactSupportTier(item.account)
				if tier == 2 {
					baseScore += 100.0 // supported accounts get priority
				}
			}
...
func (s *OpenAIGatewayService) selectAccountWithScheduler(
	...
	return s.SelectAccountWithSchedulerForPlatform(ctx, groupID, previousResponseID, sessionHash, requestedModel, excludedIDs, requiredTransport, PlatformOpenAI, requireCompact)
}
...
func (s *OpenAIGatewayService) SelectAccountWithSchedulerForPlatform(
	...
	if scheduler == nil {
		// No scheduler available — use the standard load-awareness fallback.
		// Platform routing only works with the scheduler, so we fall back to
		// the default account selection path.
