...
// SoraPriceConfig Sora 计费配置
type SoraPriceConfig struct {
	ImagePrice360          *float64 // 360p 图片价格（nil 表示使用默认值）
	ImagePrice540          *float64 // 540p 图片价格（nil 表示使用默认值）
	VideoPricePerRequest   *float64 // 视频每次请求价格（nil 表示使用默认值）
	VideoPricePerRequestHD *float64 // HD 视频每次请求价格（nil 表示使用默认值）
}

// CalculateSoraImageCost 计算Sora图片生成费用
func (s *BillingService) CalculateSoraImageCost(imageSize string, imageCount int, groupConfig *SoraPriceConfig, rateMultiplier float64) *CostBreakdown {
	...
	unitPrice := s.getSoraImageUnitPrice(imageSize, groupConfig)
...
func (s *BillingService) getSoraImageUnitPrice(imageSize string, groupConfig *SoraPriceConfig) float64 {
	if groupConfig != nil {
		switch imageSize {
		case "360":
			if groupConfig.ImagePrice360 != nil {
				return *groupConfig.ImagePrice360
			}
		case "540":
			if groupConfig.ImagePrice540 != nil {
				return *groupConfig.ImagePrice540
			}
		}
	}
	// Sora 默认图片价格
...
// CalculateSoraVideoCost 计算Sora视频生成费用
func (s *BillingService) CalculateSoraVideoCost(billingModel string, groupConfig *SoraPriceConfig, rateMultiplier float64) *CostBreakdown {
	unitPrice := s.getSoraVideoUnitPrice(billingModel, groupConfig)
	totalCost := unitPrice
	if rateMultiplier < 0 {
		rateMultiplier = 0
	}
...
func (s *BillingService) getSoraVideoUnitPrice(billingModel string, groupConfig *SoraPriceConfig) float64 {
	if groupConfig != nil {
		// HD video for opus/sonnet tier models
		if strings.Contains(billingModel, "opus") || strings.Contains(billingModel, "sonnet") {
			if groupConfig.VideoPricePerRequestHD != nil {
				return *groupConfig.VideoPricePerRequestHD
			}
		}
		if groupConfig.VideoPricePerRequest != nil {
			return *groupConfig.VideoPricePerRequest
		}
	}
	// Sora 默认视频价格
	if strings.Contains(billingModel, "opus") || strings.Contains(billingModel, "sonnet") {
